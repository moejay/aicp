...
def get_scriptwriter(scriptwriter_id):
    ...
    file = read_file(
        os.path.join(SCRIPTWRITERS_PATH, f"{scriptwriter_id.lower()}.yaml")
    )
...
def get_project_script(project_id: str):
    ...
    script = AICPScript.model_validate(
        yaml.load(
            read_file(os.path.join(script_dir, "script.yaml")), Loader=yaml.FullLoader
        )
    )
    return script
