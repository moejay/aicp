...
def list_projects():
    ...
    for filename in os.listdir(settings.AICP_OUTPUT_DIR):
        if os.path.isdir(os.path.join(settings.AICP_OUTPUT_DIR, filename)) and os.path.exists(os.path.join(settings.AICP_OUTPUT_DIR, filename, "project.yaml")):
            # Load yaml
            project_contents = read_file(os.path.join(settings.AICP_OUTPUT_DIR, f"{filename}", "project.yaml"))
...
def get_project(project_id):
    ...
    file = read_file(os.path.join(settings.AICP_OUTPUT_DIR, f"{project_id}", "project.yaml"))
