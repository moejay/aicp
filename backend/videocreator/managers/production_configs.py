...
def list_production_configs():
    ...
    for filename in os.listdir(PRODUCTION_CONFIG_PATH):
        if filename.endswith(".yaml"):
            # Load yaml
            program_contents = read_file(
                os.path.join(PRODUCTION_CONFIG_PATH, f"{filename}")
            )
            production_config = yaml.load(program_contents, Loader=yaml.FullLoader)
            production_configs.append(
                AICPProductionConfig.model_validate(production_config)
            )
...
def get_production_config(production_config_id):
    ...
    file = read_file(
        os.path.join(PRODUCTION_CONFIG_PATH, f"{production_config_id}.yaml")
    )
