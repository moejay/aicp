...
class ArtDirectorChain(Chain):
    ...
    def _call(
        ...
        storyboard_artist = AICPStoryboardArtist.model_validate(
        ...
        for seq_idx, sequence in enumerate(script.sequences, start=1):
            if os.path.exists(os.path.join(self.tmp_path, f"seq-{seq_idx}.json")):
                outline.sequences.append(
                    AICPSequence.model_validate(
                        json.load(
                            open(os.path.join(self.tmp_path, f"seq-{seq_idx}.json"))
                        )
                    )
                )
                continue
            ...
            for scene_idx, scene in enumerate(sequence.scenes, start=1):
                if os.path.exists(
                    os.path.join(self.tmp_path, f"seq-{seq_idx}-scene-{scene_idx}.json")
                ):
                    seq.scenes.append(
                        AICPScene.model_validate(
                            json.load(
                                open(
                                    os.path.join(
                                        self.tmp_path,
                                        f"seq-{seq_idx}-scene-{scene_idx}.json",
                                    )
                                )
                            )
                        )
                ...
                def gen():
                    if run_manager:
                        run_manager.on_text(
                            f"Generating shots for scene {scene_idx} in sequence {seq_idx}"
                        )
                ...
                with open(
                    os.path.join(
                        self.tmp_path, f"seq-{seq_idx}-scene-{scene_idx}.json"
                    ),
                    "w",
                ) as f:
        ...
        for sequence in outline.sequences:
            for scene in sequence.scenes:
                for shot in scene.shots:
                    ...
                    shot.layers.append(
                        VideoclipLayerAgent().generate(project, shot, storyboard_artist)
                    )
                    if shot.dialog_character in cast.keys():
                        shot.layers.append(
                            LayerVoiceoverAgent().generate(project, shot, cast=cast)
                        )
