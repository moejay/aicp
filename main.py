...
def prep_video_params(prompt, program: str, director: str, actors: str, config: str, working_dir, step):
    ...
    video = Video(
            ...
            production_config=ProductionConfig.from_yaml(os.path.join(utils.PRODUCTION_CONFIG_PATH, config + ".yaml")),
            program=Program.from_yaml(os.path.join(utils.PROGRAMS_PATH_PREFIX, program + ".yaml")),
    ...
    if args.ui:
        ...
        while retries > 0:
            try:
                ...
                output: {args.output}
                """)
                result = prep_video_params(args.prompt, args.program, args.director, args.actors, args.production_config, args.output, "Researcher")
                print(result)
                break
